...
const { mockUtils, mockStandardizedError } = createMocks();
...
jest.mock('../../standardizedError', () => mockStandardizedError);
...
describe('Socket.IO 入口點模組', () => {
  ...
  beforeEach(() => {
    ...
    mockUtils.Session.createUserIdSessionIdMap.mockReset();
    mockUtils.Session.createUserIdSocketIdMap.mockReset();
  });
  ...
  describe('JWT 驗證中間件', () => {
    it('應該在 JWT 成功驗證時繼續處理', () => {
      ...
      // 設置 JWT 驗證成功
      mockUtils.JWT.verifyToken.mockReturnValue({
        valid: true,
        userId: 'user-id-123',
      });
      ...
      expect(mockUtils.Session.createUserIdSessionIdMap).toHaveBeenCalledWith(
        'user-id-123',
        'mocked-uuid-v4',
      );
    ...
    it('應該在 JWT 有效但無用戶 ID 時拋出錯誤', () => {
      ...
      // 驗證結果
      expect(mockUtils.JWT.verifyToken).toHaveBeenCalledWith(
        'valid-jwt-no-userid',
      );
      expect(next).toHaveBeenCalledWith(
        expect.objectContaining({
  ...
  describe('事件處理註冊', () => {
    it('應該在連線時註冊所有處理器', () => {
      ...
      // 驗證用戶連線處理
      expect(mockUserHandler.connectUser).toHaveBeenCalledWith(
        mockIo,
        mockSocket,
      );
      ...
      // 驗證關鍵事件註冊
      // 使用者相關
      expect(mockSocket.on.mock.calls).toContainEqual([
        'searchUser',
        expect.any(Function),
      ]);
      expect(mockSocket.on.mock.calls).toContainEqual([
